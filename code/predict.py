...
def predict_single_image(img_path, model_rpn, model_classifier_only, cfg, class_mapping,detected):
    st = time.time()
    ...
    result_path = '/content/gdrive/My Drive/SOP/Results_Images/{}.jpg'.format(os.path.basename(img_path).split('.')[0])
...
def predict(args_):
    ...
    with open('/content/gdrive/My Drive/SOP/keras_frcnn_updated/config.pickle', 'rb') as f_in:
    ...
    if os.path.isdir(path):
        for idx, img_name in enumerate(sorted(os.listdir(path))):
            ...
            predict_single_image(os.path.join(path, img_name), model_rpn,
                                 model_classifier_only, cfg, class_mapping,detected)
        ...
        predict_single_image(path, model_rpn, model_classifier_only, cfg, class_mapping,detected)


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--path', '-p', default='/content/gdrive/My Drive/SOP/Dataset/InsulatorDataSet/Defective_Insulators/images', help='image path')
